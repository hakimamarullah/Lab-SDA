...
public class TP3d{
    ...
    public static int simulasi(Karyawan[] karyawanPangkat, int N){
        int counter = N;
        int[] flag = new int[karyawanPangkat.length];
        flag[0] = 1;
        for(int i=1; i<flag.length; i++){
           if(karyawanPangkat[i].isResign){
            flag[i] = 1;
        ...
        for(Karyawan curr: karyawanPangkat){
            if(curr != null && curr.friendsList != null){
                if(curr.friendsList.storage.size() == 0){
                    continue;
                }
                else if(curr.friendsList.getMax().pangkat > curr.pangkat && flag[curr.friendsList.getMax().pangkat] != 1){
                    flag[curr.pangkat] = 1;
    ...
    int carry(){
        ...
        try{
        if(s.size() != 0 && listKaryawan.get(s.getMax().nomor-1).pangkat != 0){
            return s.getMax().pangkat;
        }
        if(listKaryawan.get(s.getMax().nomor-1).pangkat == 0){
            s.percolateDown(0);
            return s.getMax().pangkat;
        }
        }
...
static class Network{
    ...
    int boss(Karyawan s){
        ...
        myQueue.add(s);
        flagTable[s.nomor] = 1;
        int pangkatTertinggi = Integer.MIN_VALUE;
        while(myQueue.isEmpty() == false){
            Karyawan now = myQueue.poll();
            //TP3.out.print(now.pangkat + " - ");
             if(now.pangkat > pangkatTertinggi && now != s){
                pangkatTertinggi = now.pangkat;
            }
