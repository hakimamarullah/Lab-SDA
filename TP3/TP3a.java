...
class Karyawan extends TP3a implements Comparable<Karyawan>{
    int pangkat;
    int nomor;
    int heapIndex;
    ...
    int carry(){
        ...
        if(s.size() != 0 && TP3a.listKaryawan.get(s.getMax().nomor).pangkat != 0){
            // System.out.print(s.storage);
            // System.out.println(this.nomor);
            // if(this.nomor ==10){
            //     System.out.println(s.storage.get(0).heapIndex);
            // }
            return s.getMax().pangkat;
        }
        if(TP3a.listKaryawan.get(s.getMax().nomor).pangkat == 0){

            s.percolateDown(0);

            // if(this.nomor == 10){
            //     System.out.print(s.getMax().heapIndex);
            //     for(Karyawan i : s.storage)
            //         System.out.print(i.nomor + " " + i.pangkat + " ");
            // }
            // System.out.println(" HMM "+ this.nomor);
            return s.getMax().pangkat;
        }
    }
    catch(Exception e){
    ...
    void resign(){
        ...
        for(Karyawan x: this.destList){
            ...
            if(x.friendsList != null){
                x.friendsList.percolateDown(this.heapIndex);
            }
        }
            catch(Exception e){
...
class VectorHeap{
   ...
   private void percolateUp(int leaf){
            ...
            while(now > 0 && storage.get(parent).compareTo(value) == -1){
                ...
                //System.out.print(storage.get(parent).pangkat + "-");
